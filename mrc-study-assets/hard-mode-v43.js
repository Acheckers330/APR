/* MRC v4.3 hard-mode upgrades: balanced answers, shuffled full exam, cleaner test screen */
(function(){
  ...
  function remixBalanced(qs){
    const targets=[];
    let last=-1;
    while(targets.length<qs.length){
      let block=shuffledCopy([0,1,2,3]);
      if(block[0]===last){
        const swap=block.findIndex(v=>v!==last);
        [block[0],block[swap]]=[block[swap],block[0]];
      }
      for(const v of block){
        if(targets.length<qs.length){targets.push(v);last=v;}
      }
    }
  ...
  examMenu=function(){
    ...
    menuBody.innerHTML='<button class="opt" onclick="startExam()"><b>Full 110-Question MRC Simulation</b><br><span class="muted">Shuffled questions • A/B/C/D evenly mixed • 165 minutes</span></button>';
    show('menu');
  };
  ...
  try{
    ...
    if(version) version.textContent='MRC Study System v4.3 • harder mixed-answer test build';
    const banner=document.querySelector('.banner');
    if(banner) banner.textContent='Harder test mode: questions are shuffled, A/B/C/D answers are evenly mixed, and category clues are hidden.';
    ...
    if(fullTile){
      ...
      if(p) p.textContent='Harder 110-question simulation with shuffled questions and evenly mixed A/B/C/D answers.';
    }
  }catch(e){}
})();
